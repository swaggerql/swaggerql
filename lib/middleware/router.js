...
class Router {
    constructor(logger, options) {
        ...
        this.queryBuilder = new QueryBuilder(logger, options)
        this.logger = logger
            ...
            .then(() => {
                self.api = new OpenAPIBackend({
                    ...
                    handlers: {
                        ...
                        notImplemented: async (c, ctx) => self.queryBuilder.handler(c, ctx)
                    }
                })
    ...
    getDefinition(api, options) {
        for (const path in api.paths) {
            for (const method in api.paths[path]) {
                ...
                /* Wrap query as code in the description */
                if (!this.queryBuilder.hasWrappedQuery(operation.description)) {
                    operation.description = this.queryBuilder.wrapQuery(operation.description)
                }
            }
        }
